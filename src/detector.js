...
const RAW_PATTERN_INTENTS = [
  ...
  { pattern: /vim\.lsp\.buf\.remove_workspace_folder/, intent: 'lsp.remove_workspace_folder' },
  ...
  { pattern: /vim\.cmd\.Git\b(?!.*\b(push|pull|commit|add|status|blame)\b)/, intent: 'git.fugitive' },
  ...
  { pattern: /telescope\.builtin\.current_buffer_fuzzy_find/, intent: 'navigation.buffer_search' },
  ...
  { pattern: /"_dP/, intent: 'editing.replace_with_yank' },
];
...
export function detectIntent(keymap = {}) {
  ...
  if (commandIntent) {
    return result(commandIntent, 'high', true);
  }

  const sourceIntent = detectSourceIntent({ lhs, rhsSource, desc });
  if (sourceIntent) {
    return result(sourceIntent, 'high', true);
  }
  ...
  if (descIntent) {
    return result(descIntent, 'medium', true);
  }

  return result(null, 'low', false);
}

export function detectIntents(keymaps = []) {
  return keymaps.map((keymap) => {
    const { intent, confidence, translatable } = detectIntent(keymap);
    return {
      mode: keymap.mode ?? 'n',
      lhs: keymap.lhs ?? '',
...
function result(intent, confidence, translatable) {
  return { intent, confidence, translatable };
}
...
function detectFuzzyDescIntent(desc) {
  ...
  const rules = [
    ...
    { pattern: /\b(document diagnostics?|workspace diagnostics?|diagnostics?)\b/, intent: 'navigation.diagnostics' },
    { pattern: /\b(resume( last)? search)\b/, intent: 'navigation.resume_search' },
    { pattern: /\b(next todo comment|todo next)\b/, intent: 'todo.next' },
    { pattern: /\b(previous todo comment|todo prev|todo previous)\b/, intent: 'todo.prev' },
