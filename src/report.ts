import type { ReportItem, ReportOptions } from './types.js';

function formatKeymap(item: ReportItem): string {
    const mode = item.mode ? `[${item.mode}] ` : '';
    const target = item.command ?? item.raw_rhs ?? item.intent ?? '??';
    return `${mode}${item.lhs ?? '<unknown>'} -> ${target}`;
}

export function generateReport(data: ReportOptions = {}): string {
    ...
    const manualPlugin = Array.isArray(data.manualPlugin) ? data.manualPlugin : [];
    ...
    const total =
        Number.isFinite(data.total) && typeof data.total === 'number'
            ? data.total
            : translated.length +
              pureVim.length +
              manualPlugin.length +
    ...
    lines.push(`Total detected: ${total}`);
    lines.push('');
